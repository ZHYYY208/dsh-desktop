...
const setupNodeRuntime = require(path.join(__dirname, 'setup-node-runtime.cjs'));
...
(async () => {
  // Wait for the bundled node download/extract to finish (setup-node-runtime
  // runs asynchronously; a fresh checkout has no cached node-runtime yet).
